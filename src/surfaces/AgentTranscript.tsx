        ...
        {visibleTurns.map((turn, turnIndex) => {
          ...
          const foldTitle: ReactNode = live ? (
            <LiveFoldTitle
              ...
              waitingLabel={pendingQuestion ? t("Waiting for answers") : undefined}
...
function formatWorkingDuration(
  ...
  if (elapsed == null) {
    if (done) return who ? t("{0} {1}", [who, verb]) : verb;
    return who ? t("{0} {1}…", [who, verb]) : `${verb}…`;
  }
  return who
    ? t("{0} {1} for {2}", [who, verb, elapsed])
    : t("{1} for {2}", [verb, elapsed]);
}

...
function workSummary(
  ...
  switch (kind) {
    case "edit":
      return t(live ? "Editing {0}" : "Edited {0}", [fileLabel(tally.edits)]);
    case "research":
      if (tally.reads.size > 0 && tally.searches === 0) {
        return t(live ? "Reading {0}" : "Read {0}", [fileLabel(tally.reads)]);
      }
      if (tally.reads.size === 0) {
        return live ? t("Searching the project") : t("Searched the project");
      }
      return live ? t("Exploring the project") : t("Explored the project");
    case "run":
      return tally.runs === 1
        ? live
          ? t("Running a command")
          : t("Ran a command")
        : t(live ? "Running {0} commands" : "Ran {0} commands", [tally.runs]);
    case "agent":
      return tally.agents === 1
        ? live
          ? t("Running a subagent")
          : t("Ran a subagent")
        : t(live ? "Running {0} subagents" : "Ran {0} subagents", [tally.agents]);
    default:
      return tally.others === 1
        ? live
          ? t("Running a tool")
          : t("Ran a tool")
        : t(live ? "Running {0} tools" : "Ran {0} tools", [tally.others]);
  }
}

...
export type KimiModeId = "default" | "plan" | "auto" | "yolo";

export type KimiPermissionRequest = {
  title: string;
  kind?: string;
  callId?: string;
  preview?: ToolPreview;
...
export function kimiPromptBlocks(
  text: string,
...
export function kimiModeId(
  runtimeMode: RuntimeMode,
  planning = false,
): KimiModeId {
  if (planning) return "plan";
  if (runtimeMode === "full-access") return "yolo";
  if (runtimeMode === "auto") return "auto";
  // Kimi has no edit-only mode; approve edit requests on the client instead.
  return "default";
}

...
import * as kimi from "./kimiProtocol";

const providers = [
  { id: "kimi", protocol: kimi, mode: kimi.kimiModeId,
    blocks: kimi.kimiPromptBlocks,
    modes: ["default", "default", "auto", "yolo"], plan: "plan" },
] as const;
...
describe.each(providers)("$id ACP protocol", ({ id, protocol, mode, blocks, modes, plan }) => {
  ...
  it("falls back to top-level ACP models without inventing reasoning controls", () => {
    expect(protocol.modelsFromSessionNew({ models: { availableModels: [
      ...
      id: `${id}:kimi-code/k3`, harness: id, nativeId: "kimi-code/k3",
      name: "K3",
    }]);
  });

...
// The server answers session/prompt only after the turn fully ends, so a
// wedged server may never resolve it. Silence is ambiguous — a long quiet tool
// call is legitimate — so past this point we surface a status note instead of
// killing the turn.
...
function noteActivity(live: Live): void {
  ...
  live.watchdog = setTimeout(() => {
    ...
    live.onEvent({
      type: "status",
      text: "Kimi Code has been quiet for two minutes — its post-turn work may be stuck. Stop and resend to recover.",
    });
  }, STALL_NOTIFY_MS);
}
...
function queueLifecycle(input: SendTurnInput, recycle: boolean): Promise<Live> {
  ...
  const next = prev.then(async () => {
    if ((sessionEpoch.get(input.sessionId) ?? 0) !== life) {
      throw new Error("Kimi Code session superseded");
    }
...
async function startLive(input: SendTurnInput, life: number): Promise<Live> {
  ...
  if (retired()) throw new Error("Kimi Code session stopped during startup");
  ...
  try {
    ...
    if (!acpSessionId) {
      ...
      if (acpSessionId && droppedBinding) {
        emit({
          type: "status",
          text: "Kimi Code could not restore the previous conversation — starting a new session.",
        });
      }
    }
    ...
    // thread slot, so nothing here may touch the thread-keyed maps.
    acp.close(error instanceof Error ? error : new Error(String(error)));
    unwatchChild(childKey);
...
async function prompt(live: Live, input: SendTurnInput): Promise<void> {
  try {
    ...
    if (stopReason != null && stopReason !== "end_turn") {
      live.onEvent({
        type: "session.error",
        message: `Kimi Code ended the turn (${String(stopReason)}).`,
      });
      return;
    }

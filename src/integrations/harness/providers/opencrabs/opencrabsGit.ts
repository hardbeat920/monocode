...
import { runOpenCrabsTextPrompt } from "./opencrabsText";

const GIT_TIMEOUT_MS = 90_000;

export async function generateOpenCrabsCommitMessage(
  cwd: string,
): Promise<string> {
  const context = await gitStagedContext(cwd);
  const output = await runOpenCrabsTextPrompt({
    cwd,
  ...
  throw new Error(
    snippet
      ...
      : "Could not generate a commit message. OpenCrabs returned no text.",
  );
}

export async function generateOpenCrabsPrContent(
  cwd: string,
  ...
  try {
    ...
    console.debug("[monocode] opencrabs pr content", error);
  }
  const title =
    parsed?.title ||
...
export async function generateOpenCrabsBranchName(
  cwd: string,
  message: string,
): Promise<string | null> {
  try {
    ...
    console.debug("[monocode] opencrabs branch name", error);
    return null;
  }
}

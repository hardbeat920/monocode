...
function mapSystem(
  ...
  if (stringField(rec, "subtype") !== "turn_duration") return [];
...
/**
 * End a turn on the authority of the pty screen rather than a record.
 *
 * This is the other half of the interrupt contract: an interrupt taken on the
 * phone or in the TUI writes nothing, so the only evidence the turn is over is
 * the composer going idle. Passing `idle: false` is a no-op, so a caller can
 * feed screen state unconditionally.
 */

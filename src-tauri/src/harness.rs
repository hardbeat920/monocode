...
fn mcode_help_mentions_acp(path: &Path) -> bool {
    ...
    match rx.recv_timeout(Duration::from_secs(2)) {
        Ok(Ok(output)) => {
            // Some harnesses print help on stdout, others on stderr; merge
            // both streams before matching so we don't false-negative on
            // the ones that route help to stderr (matches fx_help_mentions_acp).
            let text = format!(
                "{}{}",
                String::from_utf8_lossy(&output.stdout),
                String::from_utf8_lossy(&output.stderr)

...
pub fn harness_resolve_devin() -> Result<CursorBinary, String> {
    resolve_devin()
        ...
        .ok_or_else(|| {
            "Devin CLI not found. Install it from https://devin.ai/cli, run `devin auth login`, then retry."
                .into()
        })
}
...
fn resolve_devin() -> Option<PathBuf> {
    // The official installer places `devin` on PATH on macOS, Linux, and
    // Windows. `resolve_gui_binary` reconstructs the terminal PATH for GUI
    // launches and already covers ~/.local/bin, Homebrew, npm, Scoop, etc.
    resolve_gui_binary("devin")
}
